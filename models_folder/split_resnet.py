...
class ResNetVSplit(nn.Module):
    ...
    def set_grad_status(self):
        ...
        if self.split_layer >= 4:
            for name, param in self.layer4.named_parameters():
                param.requires_grad = False
            self.layer4.apply(self.set_bn_eval)

            
    def __init__(self,cfg, builder, block, layers, split_layer, base_width=64):

        super(ResNetVSplit, self).__init__()
        self.inplanes = 64
        slim_factor = cfg.slim_factor
        if slim_factor < 1:
            cfg.logger.info('WARNING: You are using a slim network')
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        if self.split_layer == 0:
            x = x.normal_(torch.mean(x), torch.std(x))
        ...
        x = self.layer1(x)
        if self.split_layer == 1:
            x = x.exponential_(1/torch.mean(x))
        # features.append(x)

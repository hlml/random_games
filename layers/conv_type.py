    ...
    def split_reinitialize(self,cfg):
        ...
        if hasattr(self, "bias") and self.bias is not None and self.bias_split_rate < 1.0:
            ...
            if cfg.evolve_mode == 'rand':
                ...
                rand_tensor = torch.zeros_like(self.bias)
                self.weight.data = torch.where(bias_mask.type(torch.bool), self.bias.data, rand_tensor)
            else:
                raise NotImplemented('Invalid KE mode {}'.format(cfg.evolve_mode))
...
class MaskConv(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        self.mask = nn.Parameter(module_util.mask_init(self))
    ...
    def forward(self, x):
        subnet = module_util.GetSubnet.apply(self.mask.abs(), self.split_rate)
